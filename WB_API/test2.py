...
df = pd.DataFrame(stocks) # датафрейм со стоками
df_orders = pd.DataFrame(orders)  #датафрейми с продажами
df['В пути до получателей'] = df.apply(lambda x: sum(x['warehouses'][i]['quantity'] for i in range(len(x['warehouses'])) if x['warehouses'][i]['warehouseName'] == 'В пути до получателей'), axis = 1)
df['Возвраты в пути'] = df.apply(lambda x: sum(x['warehouses'][i]['quantity'] for i in range(len(x['warehouses'])) if x['warehouses'][i]['warehouseName'] == 'В пути возвраты на склад WB'), axis = 1)
df['Всего находится на складах'] = df.apply(lambda x: sum(x['warehouses'][i]['quantity'] for i in range(len(x['warehouses'])) if x['warehouses'][i]['warehouseName'] == 'Всего находится на складах'), axis = 1)
df['Москва'] = df.apply(lambda x: sum(x['warehouses'][i]['quantity'] for i in range(len(x['warehouses'])) if x['warehouses'][i]['warehouseName'] == 'Электросталь' or x['warehouses'][i]['warehouseName'] == 'Коледино' or x['warehouses'][i]['warehouseName'] == 'Белые Столбы'), axis = 1)
df['Казань + Екат'] = df.apply(lambda x: sum(x['warehouses'][i]['quantity'] for i in range(len(x['warehouses'])) if x['warehouses'][i]['warehouseName'] == 'Казань' or x['warehouses'][i]['warehouseName'] == 'Екатеринбург - Перспективный 12'), axis = 1)
...
df_full = df_mapping.merge(df[['barcode','Всего находится на складах','Возвраты в пути']], left_on = 'barcode', right_on = 'barcode', how = 'left')
